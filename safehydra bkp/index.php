...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate') {
    ...
    $stmt->execute([$customer, $company, $amount, $doc_number]);
    $new_id = $pdo->lastInsertId();

    echo json_encode(['success' => true, 'id' => $new_id, 'doc_number' => $doc_number, 'customer_name' => $customer, 'company_name' => $company, 'amount' => number_format($amount, 2), 'created_at' => date('d-M-Y H:i')]);
    exit;
}
...
.download-btn{display:inline-flex;align-items:center;gap:5px;padding:5px 12px;background:#f97316;color:#fff;border-radius:6px;font-size:11.5px;font-weight:700;text-decoration:none;transition:background .15s;}
...
<div class="page-wrap">
  ...
  <div class="content">
    ...
    <div class="table-card">
      ...
      <table id="docsTable">
        <thead>
          <tr><th>#</th><th>Doc Number</th><th>Customer Name</th><th>Company Name</th><th>Amount (₹)</th><th>Created</th><th>Action</th></tr>
        </thead>
        <tbody id="tableBody">
          <?php if(empty($docs)): ?>
          <tr><td colspan="7"><div class="empty-state"><i class="fas fa-folder-open"></i><p>No documents yet.</p></div></td></tr>
          ...
          <tr>
            ...
            <td><a href="/invoice/safehydra/download_sh.php?id=<?= $d['id'] ?>" class="download-btn"><i class="fas fa-download"></i> Download PDF</a></td>
          </tr>
...
function closeModal(){document.getElementById('genModal').classList.remove('show');clearFields();}
...
function generateDoc(){
  ...
  fetch('/invoice/safehydra/index.php',{method:'POST',body:fd})
    .then(r=>r.json()).then(data=>{
      if(data.success){
        closeModal();showToast('Document '+data.doc_number+' generated!');
        const tbody=document.getElementById('tableBody');
        const empty=tbody.querySelector('.empty-state');if(empty)tbody.innerHTML='';
        const rows=tbody.querySelectorAll('tr');
        tbody.insertAdjacentHTML('afterbegin',`<tr>
          <td>${rows.length+1}</td>
          <td><span class="doc-num">${data.doc_number}</span></td>
          <td><span class="customer-name">${data.customer_name}</span></td>
          <td>${data.company_name}</td>
          <td><span class="amount-cell">₹${data.amount}</span></td>
          <td><span class="date-cell">${data.created_at.split(' ')[0]}</span></td>
          <td><a href="/invoice/safehydra/download_sh.php?id=${data.id}" class="download-btn"><i class="fas fa-download"></i> Download PDF</a></td>
        </tr>`);
