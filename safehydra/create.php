...
.preview-modal{background:#fff;border-radius:0;width:100vw;max-width:none;height:100vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:none;}
...
<!-- Preview Modal — loads original template via preview.php -->
<div class="preview-overlay" id="previewOverlay">
  <div class="preview-modal">
    ...
    <iframe src="" id="previewFrame" title="Safe Hydra Template Preview"></iframe>
  </div>
</div>
...
function openPreview(){
  // Load the template PDF (original, unmodified) in the iframe
  document.getElementById('previewFrame').src = '/invoice/safehydra/preview.php';
...
function generateDoc(){
  ...
  fetch('/invoice/safehydra/index.php',{method:'POST',body:fd})
    .then(r=>r.json()).then(data=>{
      if(data.success){
        closeGenerate();
        showToast('Document '+data.doc_number+' generated successfully.');
